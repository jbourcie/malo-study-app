# es_vocab_daily_life

## Métadonnées
- Label: Vocabulaire du quotidien
- Matière (subject): es
- Thème (theme): Vocabulaire
- Ordre (order): 10

## Compétence
(À compléter) Objectif de la notion « Vocabulaire du quotidien ».

## Définition {#definition}
(À compléter) Définition courte, niveau 5e, pour « Vocabulaire du quotidien ».

## Méthode {#methode}
(À compléter) Étapes/règles à appliquer pour réussir sur « Vocabulaire du quotidien ».

## Exemples {#exemples}
- (À compléter) Exemple 1
- (À compléter) Exemple 2
- (À compléter) Exemple 3

## Pièges fréquents {#pieges}
- (À compléter) Erreur fréquente 1
- (À compléter) Erreur fréquente 2
- (À compléter) Erreur fréquente 3

## Variantes de questions autorisées
- MCQ
- TRUE_FALSE

## Interdictions
- FILL_BLANK (texte à trou) si la réponse peut être ambigüe
- Questions ouvertes / réponses longues
