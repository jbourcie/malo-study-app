  ...
  return (
    <div className={`card mc-card ${className || ''}`}>
      <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {onChangeNpc && (
          <button className="mc-button secondary" onClick={onChangeNpc}>Changer</button>
        )}
      </div>

      {loading ? (
        <div className="mc-card" style={{ marginTop: 8, border: '2px solid var(--mc-border)' }}>
          <div className="small" style={{ color: 'var(--mc-muted)' }}>Chargement des quêtes…</div>
        </div>
      ) : allCompleted ? (
        <div className="mc-card" style={{ marginTop: 8, border: '2px solid var(--mc-border)', display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div className="small" style={{ color: 'var(--mc-muted)' }}>
            {npc.shortTagline || npc.name} : Reviens demain pour de nouvelles missions.
          </div>
        </div>
      ) : (
        <div className="mc-card" style={{ marginTop: 8, border: '2px solid var(--mc-border)', display: 'flex', flexDirection: 'column', gap: 10 }}>
